...
public class AesService
{
    ...
    // AesService.cs
    public byte[] Encrypt(PhysicalContactData data)
    {
        var json = JsonSerializer.Serialize(data);
        return EncryptString(json);
    }

    public PhysicalContactData DecryptPhysical(byte[] encrypted)
    {
        var json = DecryptToString(encrypted);
        return JsonSerializer.Deserialize<PhysicalContactData>(json);
    }
}